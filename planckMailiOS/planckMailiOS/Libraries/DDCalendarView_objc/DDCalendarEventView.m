...
- (id)initWithEvent:(DDCalendarEvent*)event {
    ...
    if(self) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(3, 3, self.bounds.size.width-6, 15)];
        
        
        UIFont * customFont = [UIFont fontWithName:@"Helvetica" size:11]; //custom font
        label.font = customFont;
        UIColor *c = [UIColor blackColor];
        if(event.userInfo[@"color"]) {
            c = event.userInfo[@"color"];
        }
        CGFloat hue, saturation, brightness, alpha ;
        BOOL ok = [c getHue:&hue saturation:&saturation brightness:&brightness alpha:&alpha ] ;
        if ( ok ) {
            c = [ UIColor colorWithHue:hue saturation:saturation brightness:brightness*0.6 alpha:alpha ] ;
        }
...
- (void)layoutSubviews {
    ...
    CGFloat y = selfFrame.origin.y<0?-selfFrame.origin.y+3:3;
    self.label.frame = CGRectMake(3, y, selfFrame.size.width-6, 15);
}
